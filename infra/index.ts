...
// import {
//   GuardDutyComponent,
//   GuardDutyComponentArgs,
// } from "./modules/guardduty";

// // Define the arguments for GuardDutyComponent
// const guardDutyArgs: GuardDutyComponentArgs = {
//   tags: { Environment: "Guardduty New Betsy" },
//   orgAccountId: config.orgAccountId,
//   autoEnableOrganizationMembers: "NEW",
// };

// // Instantiate the GuardDutyComponent
// const guardDuty = new GuardDutyComponent("myGuardDutyComponent", guardDutyArgs);

// // Export the outputs for Pulumi
// export const organizationAdminAccountId = guardDuty.organizationAdminAccount.id;
// export const detectors = guardDuty.detectors.map((detector) => detector.id);
// export const detectorIds = guardDuty.detectors.map((detector) => detector.id
// );

import * as pulumi from "@pulumi/pulumi";
import * as aws from "@pulumi/aws";
import * as config from "./config";
import { GuardDutyComponent } from "./modules/guardduty";

// Pulumi configuration
const pulumiConfig = new pulumi.Config();
const orgAccountId = pulumiConfig.require("orgAccountId"); // Ensure this is set in Pulumi stack config
const autoEnableOrganizationMembers = pulumiConfig.get("autoEnableOrganizationMembers") || "NEW";

// Log the AWS account being used
aws.getCallerIdentity().then((identity) => {
  pulumi.log.info(`Using AWS Account: ${identity.accountId}`);
});

// GuardDuty Setup
const guardDuty = new GuardDutyComponent("guardDuty", {
  orgAccountId: orgAccountId, // Organization admin account ID
  autoEnableOrganizationMembers: autoEnableOrganizationMembers, // Enable auto-renew for new accounts
});

// Export Outputs
export const outputs = {
  organizationAdminAccountId: guardDuty.organizationAdminAccount.id,
  detectorIds: guardDuty.detectors.map(detector => detector.id),
};
